...
class MonthWiseAnalyzePage extends StatefulWidget {
  const MonthWiseAnalyzePage({super.key});
...
class _MonthWiseAnalyzePageState extends State<MonthWiseAnalyzePage> {
  ...
  void _updateGraphData(DataModel dataModel) {
    for (int i = 0; i < 12; i++) {
      final month = DateTime(DateTime.now().year, i + 1);
      monthsExpense[i] = dataModel.expenses
          .where((e) =>
      e.date.month == month.month && e.date.year == month.year)
          .fold(0.0, (sum, e) => sum + e.amount);
      monthsBudget[i] = dataModel.budgets
          .where((b) =>
    ...
    for (int i = 0; i < categories.length; i++) {
      categoriesExpense[i] = dataModel.expenses
          .where((e) => e.category == categories[i])
          .fold(0.0, (sum, e) => sum + e.amount);
      categoriesBudget[i] = dataModel.budgets
          .where((b) => b.category == categories[i])
          .fold(0.0, (sum, b) => sum + b.amount);
    }
  }
}
